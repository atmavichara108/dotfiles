...
- [ ] **Добавить пакет `kitty`** — если используется как второй терминал
- [ ] **Добавить пакет `Thunar`** — настройки файлового менеджера (GUI fallback)
...
- [ ] **Hyprland-конфиги** — задел на Wayland (уже установлен hyprland-git)
- [ ] **i3-совместимые конфиги** — manjaro-i3-settings установлен, возможно пересечение
...
- [ ] **Добавить пакет `calcurse`** — календарь в терминале
...
- [ ] **Систематизация дрейфа** — захват systemd-сервисов, wal-шаблонов, picom
- [ ] **Очистка диска** — освободить место на / (91% заполнения)
- [ ] **ADR-002: Единая система цвета** — pywal + wallust + tinty
...
| 1.3 | Пакет `kitty` | конфиг kitty (если используется) | `/util` |
| 1.4 | Пакет `calcurse` | календарь с цветовой схемой | `/util` |
| 1.5 | Пакет `copyq` | конфиг буфера обмена | `/util` |
...
| # | Задача | Ожидаемый результат | Пайплайн |
|---|--------|---------------------|----------|
| 3.1 | Hyprland-конфиги | задел на Wayland (уже установлен) | `/qtile` → `/plugin` |
| 3.2 | Docker-интеграция | контейнеры для dev-окружений | `/script` |
...
### Окружение
1. **Основной WM** — Qtile (X11). Hyprland (установлен) — для тестов или планы на миграцию?
2. **Alacritty vs kitty** — kitty тоже установлен. Какой основной? kitty для специфических задач?
...
5. **File manager** — ranger или Thunar (установлены оба)? Когда какой?
