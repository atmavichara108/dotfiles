...
которые дублировали владение и роутинг (системный оператор, архитектор,
строитель — все primary). Плюс имя `sysop` было коллизией: локальный
первичный «оператор Manjaro» и глобальный первичный «system-audit» — оба
назывались sysop и оба mode: primary. Плюс атавизм `think` — agent-блок в
`opencode.json` на `opencode/grok-build-0.1`, для которого не было ни
prompt-файла, ни роутинга (незавершённый зародыш «второго мыслителя»).
**Решение:**
1. Единственный primary — `sysop` (оператор-оркестратор, gpt-5.6-luna):
   сам анализирует/проектирует/пишет, по необходимости `task`-ает субагентов.
2. `planner` и `builder` переведены из primary в subagent (mode: subagent);
   у них убрана собственная `task`-оркестрация — субагенты не оркестрируют.
3. Субагенты из нестандартного `.opencode/subagent/` (bash-dev, qtile-dev,
   stow-ops, util-dev, verifier) перенесены в `.opencode/agent/`, чтобы
   регистрироваться авто-дискавери по `.md`-frontmatter (паттерн из vault),
   а не через agent-блоки `opencode.json`.
4. Глобальный `sysop` переименован в `system-audit` (файл
   `system-audit.md`, mode: subagent) — имя `sysop` теперь однозначно означает
   локального первичного оператора, а read-only аудит несёт отдельная роль.
5. В `opencode.json` удалены agent-блоки только моей зоны (sysop/planner/
   builder/think); `default_agent` → `sysop`. Канон агента — `.md`-frontmatter.
6. Атавизм `think` удалён целиком.
**Альтернативы:**
- Оставить три primary — отвергнуто: путаница владения, три «мозга» на один проект.
- Полная вычистка `opencode.json` от всех agent-блоков сразу — отложено:
  субагентные модельные правки из соседней сессии (linaliapi/… миграция)
  ещё не закоммичены; вычистка остальных блоков — отдельный заход после их
  консолидации в `.md`.
- Удалить planner/builder целиком, оставить только domain-dev — отвергнуто:
  стратег и общий кодер нужны sysop'у как субагенты.
**Последствия:**
- Один primary в dotfiles; иерархия плоская: `sysop → subagent`.
- `opencode agent list` через CLI не показывает локальных агентов (показывает
  только built-in + глобальные) — это особенность CLI, не регресс; локальные
  резолвятся в live TUI-сессии. Discovery проверяется после рестарта TUI.
- Требуется перезапуск OpenCode (agents/commands не hot-reload).
- `verifier` в `opencode.json` остался с моделью `linaliapi/z-ai/glm-5.3` (миграция
  соседней сессии); canonical значение — в `.opencode/agent/verifier.md`
  (`opencode/deepseek-v4-flash-free`). Рассинхрон устраняется в отдельном заходе.
